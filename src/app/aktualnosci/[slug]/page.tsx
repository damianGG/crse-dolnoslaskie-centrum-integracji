...
async function getStrapiData(id: string) {
    ...
    const data = await fetch(`${backendLink}/api/neet-aktualnoscis/${id}?populate=*`, {
        cache: 'no-store',
        method: 'GET',
        headers: {
...
export default async function BlogDetailsTemplate({ params }: { params: { slug: string } }) {
    ...
    const tekstPlikiDoPobrania = data.attributes?.TekstPliikDoPobrania || '';
    const downloadURL = data.attributes.PlikiDoPobrania?.data?.attributes?.url;

    console.log(tekstPlikiDoPobrania)
    return (
        <>
